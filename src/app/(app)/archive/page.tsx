...
export default async function Archive(){
  ...
  {reports.map(r=><tr key={r.id}><td>{r.employee.name}</td><td>{r.title}</td><td>{r.startAt.toLocaleDateString("de-DE")} – {r.endAt.toLocaleDateString("de-DE")}</td><td><span className="badge ok">{r.status}</span></td><td><Link href={`/reports/${r.id}`}>Öffnen</Link> · <a href={`/api/reports/${r.id}/pdf`}>PDF</a></td></tr>)}
