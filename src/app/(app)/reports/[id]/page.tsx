...
export default async function ReportPage({params}:{params:Promise<{id:string}>}){
  ...
  return <><div className="actions" style={{justifyContent:"space-between"}}><div><h1>{report.title}</h1><div className="sub">{report.employee.name} · {report.destination}</div></div><div className="actions"><a className="button secondary" href={withBasePath(`/api/reports/${id}/pdf`)}>PDF</a>{editable&&<><Link className="button secondary" href={`/reports/${id}/edit`}>Bearbeiten</Link><form action={remove}><input name="reportId" type="hidden" value={id}/><ConfirmDeleteButton/></form><form action={submit}><button>Zur Prüfung senden</button></form></>}</div></div>
  <section className="grid two">
    <div className="card"><h2>Reise</h2><table><tbody><tr><th>Zeitraum</th><td>{report.startAt.toLocaleString("de-DE")} – {report.endAt.toLocaleString("de-DE")}</td></tr><tr><th>Zweck</th><td>{report.purpose}</td></tr><tr><th>Verkehrsmittel</th><td>{report.transportType}</td></tr><tr><th>Status</th><td><StatusBadge status={report.status}/></td></tr></tbody></table>
