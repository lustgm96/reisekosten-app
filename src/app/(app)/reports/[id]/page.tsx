...
export default async function ReportPage({params}:{params:Promise<{id:string}>}){
  ...
  async function saveProvidedMeals(fd:FormData){"use server";const u=await requireUser();const r=await db.expenseReport.findUniqueOrThrow({where:{id}});if(r.employeeId!==u.id||!["DRAFT","RETURNED"].includes(r.status))throw new Error("Nicht erlaubt");const meals=validateProvidedMeals(fd.getAll("meals").map(String),r.startAt,r.endAt);await db.expenseReport.update({where:{id},data:{providedMealsJson:JSON.stringify(meals),...countProvidedMeals(meals),mealsReviewedAt:new Date()}});revalidatePath(`/reports/${id}`)}
  ...
  <section className="grid two">
    ...
    <h2 style={{marginTop:22}}>Belege und Ausgaben</h2><table><thead><tr><th>Datum</th><th>Kategorie</th><th>Beschreibung</th><th>Beleg</th><th>Zahlung</th>{user.role!=="EMPLOYEE"&&<th>MwSt.</th>}<th>Betrag</th>{editable&&<th>Aktionen</th>}</tr></thead><tbody>{report.expenses.map((x,index)=><tr key={x.id}><td>{x.expenseDate.toLocaleDateString("de-DE")}</td><td>{x.category}</td><td>{x.description}</td><td>{x.storedFileName?<ReceiptPreview fileName={receiptDocumentTitle(report.processNumber,x.createdAt,index)} mimeType={x.mimeType} url={withBasePath(`/api/files/${x.id}`)}/>:<span className="small">Kein Beleg</span>}</td><td>{x.paymentType}</td>{user.role!=="EMPLOYEE"&&<td>{eur.format(Number(x.vatAmount))}</td>}<td>{eur.format(Number(x.amount))}</td>{editable&&<td><div className="table-actions"><Link href={`/reports/${id}/expenses/${x.id}/edit`}>Bearbeiten</Link><form action={removeExpense}><input name="reportId" type="hidden" value={id}/><input name="expenseId" type="hidden" value={x.id}/><ConfirmDeleteButton label="Entfernen" message="Diese Ausgabe und den zugehörigen Beleg löschen?"/></form></div></td>}</tr>)}</tbody></table>
