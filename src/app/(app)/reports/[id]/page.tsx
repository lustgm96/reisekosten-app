...
export default async function ReportPage({params}:{params:Promise<{id:string}>}){
  ...
  async function addExpense(fd:FormData){"use server";const u=await requireUser();const r=await db.expenseReport.findUniqueOrThrow({where:{id}});if(r.employeeId!==u.id||!["DRAFT","RETURNED"].includes(r.status))throw new Error("Nicht erlaubt");const p=expenseSchema.parse(Object.fromEntries(fd));if(r.accommodationMode==="PER_DIEM"&&p.category==="Hotel")throw new Error("Bei Übernachtungspauschale kann kein Hotelbeleg erfasst werden.");const file=fd.get("file") as File;const upload=file?.size?await storeUpload(file):{};await db.expenseItem.create({data:{reportId:id,...p,...upload}});revalidatePath(`/reports/${id}`)}
